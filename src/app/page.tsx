...
function ClockInApp() {
  ...
  const stationId = searchParams.get("station") ?? "1"; // Default to station 1 if not provided

  const employees = STATIONS_DATA[stationId] || ["Empleado Desconocido"];
...
export default function Page() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[var(--color-background)] flex items-center justify-center text-white">Cargando...</div>}>
      <ClockInApp />
    </Suspense>
  );
}
